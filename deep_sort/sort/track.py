...
class Track:
    ...
    def update(self, kf:KalmanFilter, detection):
        ...
        """
        if(np.all(self.mean[0]== -1)):
            self.mean, self.covariance = kf.initiate(detection.to_xyahxwyw())
        det_box_coords = detection.to_xyahxwyw()
        if(det_box_coords[-1] == -1):
        ...
        if(len(self.features) > 200):
            # print("**"*50)
            # print("before", len(self.features))
            self.features = self.features[:50] + (self.features[80:-50])[::2] + self.features[-2:]
            # print("after", len(self.features))
            # print("**"*50)
        # self.preds.append(detection.pred)
        self.hits += 1
        self.time_since_update = 0
