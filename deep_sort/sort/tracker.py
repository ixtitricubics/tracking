...
class Tracker:
    ...
    def __init__(self, metric,db:Dataset, max_iou_distance=0.7, max_age=70, n_init=3):
        ...
        self.kf = kalman_filter.KalmanFilterNew()
        self.tracks = []
        self._next_id = 1
    ...
    def merge_track(self, from_id, to_id):
        ...
        for i in range(len(self.tracks)):
            if(self.tracks[i].track_id == from_id):
                from_id_ind = i
            elif(self.tracks[i].track_id == to_id):
                to_id_ind = i
        self.tracks[to_id_ind].features.extend(self.tracks[from_id_ind].features.copy())
        self.tracks[to_id_ind].time_since_update = 0
        self.tracks[to_id_ind].hits +=1
        del self.tracks[from_id_ind]

    def delete_track(self, id):
        id_ind = -1
        for i in range(len(self.tracks)):
            if(self.tracks[i].track_id == id):
                id_ind = i
        print("deleted track :", id)
        del self.tracks[id_ind]
    def change_pos(self, track_id, new_pos):
        id_ind = -1
        for i in range(len(self.tracks)):
            if(self.tracks[i].track_id == track_id):
                id_ind = i
        self.tracks[id_ind].mean[4:6] = new_pos
    ...
    def get_feat(self, track_id):
        ...
        for i in range(len(self.tracks)):
            if(self.tracks[i].track_id == track_id):
                id_ind = i
        return self.tracks[id_ind].features[-1]
    ...
    def _initiate_track(self, detection, from_db = True):
        mean, covariance = self.kf.initiate(detection.to_xyahxwyw())
    ...
    def _initiate_track_from_db(self):
        ...
        #     det.id = 2
        #     detections.append(det)
            mean, covariance = self.kf.initiate([-1,-1,-1, -1, -1,-1])
